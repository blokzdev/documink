...
- Read the relevant section(s) of `docs/DocuMink-Roadmap.md` for phase context.
- Read `docs/DocuMink-Blueprint.md` §15 "Don't do" rules. Violating these without explicit user consent is a task-stopping condition.
- For memory-layer work, also read `docs/DocuMink-Memory.md` §3 and §12.
...
5. **No Postgres or specialized databases on-device.** SQLite + SQLCipher + sqlite-vec + FTS5 + JSON1 + cr-sqlite covers every capability we need. See Blueprint §2.4 for rejected-alternatives rationale.
6. **No multi-agent Mink architectures.** Single agent, deterministic router. See ADR-012 and DocuMink-Memory.md §4.
