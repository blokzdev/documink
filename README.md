...
- **4c-1 (this PR)** — the **headless crypto/schema core**: `TokenCrypto.encryptBytes/decryptBytes` (AES-256-GCM under the vault DEK, AAD = document id); a new **`document_originals`** table storing the encrypted original as a BLOB **inside the SQLCipher DB**, added via the repo's **first drift migration** (schemaVersion 1→2, tested against the real `onUpgrade`); `OriginalsRepository` (save/fetch/decrypt, deletion wired into the document delete cascade); `OriginalRevealService` (biometric-gated decrypt, audited `document_original_revealed`); an **opt-in setting** (`keepOriginalProvider`, default off). Fully unit-tested. **4c-2 (next):** capture the original through ingestion→save, the biometric **secure viewer** (`Image.memory`/`pdfx`) with **FLAG_SECURE** + cache hygiene, the Settings toggle, and a one-time contextual "keep the original?" notice.
