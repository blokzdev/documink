...
- **13b (this PR)** — the **Settings toggle + disclosure flag**: `ProactiveSuggestionsController` (key `proactive_suggestions_enabled`, **default on / opt-out** via `!= 'false'`, mirroring `KeepOriginalController`) and `proactiveSuggestionsDisclosureSeenProvider` (one-time first-offer disclosure flag, rendered by 13c). A `SwitchListTile` (`proactive-suggestions-toggle`) lands in the Settings → Privacy section with localized strings. Controller + settings widget tests (default-on, opt-out persists).
