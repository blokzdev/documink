...
- **5d (this PR)** — **settings persistence**: a `SettingsStore` seam (in-memory default for tests; `shared_preferences`-backed impl wired at bootstrap) so the theme choice survives restarts. Non-sensitive prefs only — never PII (privacy invariant #4). The store is the reusable persistence point for later non-sensitive settings.
