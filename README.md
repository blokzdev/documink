...
- **4b (this PR)** — **PDF import + input-flow polish**: `importPdf()` extracts a PDF's text layer page-by-page (`flutter_pdf_text`/Apache PDFBox) and **falls back to OCR for scanned pages** by rasterizing them (`pdfx`) into the *existing* OCR seam; `file_selector` picks the file. Three new seams (`PdfSource`/`PdfTextExtractor`/`PdfPageRasterizer`) keep the orchestration headless-testable; real adapters wired at bootstrap, device-verified. Polish: the capture + paste-editor screens are now **localized** (ARB/`AppLocalizations`), a **source badge** + multi-page/scanned-page **warnings** surface on review (exercising `IngestedText.pageCount`/`warnings`), the Scan screen offers **"Choose from gallery"**, and the recognized-text region gets **a11y `Semantics`**. **Remaining Phase 4 (next PR):** inbound share-sheet intent.
