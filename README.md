...
- **L5 (this PR)** — **Settings & Audit elevation + motion/a11y** (closes the arc): grouped settings **cards**, audit entries with **per-event icons**, app-wide **page transitions** (zoom), and `header` semantics on section labels. Settings/detail/browser/audit/home all centred under a max content width.
