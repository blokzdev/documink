...
- **9b (this PR)** — **Ed25519-signed manifest** (§4.7 / models.md §5). `ManifestVerifier` verifies the signature over the manifest body against a **pinned** public key (rejects tampered body/signature/wrong-key/bad-alg — never falls back to unsigned), then parses it into a `ModelManifest` (`TierCatalog` + Tier 3 `DetectionModel`s). `ModelHashVerifier` does post-download SHA-256. Ships `assets/model_manifest/manifest.{json,signed.json}` + a re-runnable `tool/scripts/sign_manifest.dart`. The manifest also carries the **Tier 3 GLiNER PII** entries (bundled baseline + device-tiered upgrades) — **this unblocks the deferred Tier 3 delivery** (ADR-022). Native signal collection + `vault_meta` persistence follow in 9c.

**Next: V1 Phase 9c — native signal collector interface + `vault_meta` persistence + providers** (then Phase 6 custom entities, Phase 12 memory-safety; native/UI phases 4–5,7–8 on a device session).
